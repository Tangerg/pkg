...
func ExpandSelfCloseElement(element string) (string, string, error) {
	...
	startEle, endEle := expandSelfCloseElement(element, name.Local)
...
func parseAttrs(startEle string) ([]Attr, error) {
	...
	for pos < len(attrStr) {
		...
		if pos >= len(attrStr) {
			return nil, fmt.Errorf("attribute %q has unclosed quote", attrName)
		}
...
func ExtractElementName(eleContent string) (Name, error) {
	...
	name := extractElementName(eleContent)
	if name.Local == "" {
		return Name{}, errors.New("empty element name")
	}
...
func IsStartElement(element string) bool {
	...
	// Not an end element </element>
	if trimmed[1] == '/' {
		return false
	}

	// Not a self-closing element <element/>
...
func isValidElementSyntax(element string) bool {
	...
	// Cannot be both end element and self-closing: </element/>
	if isEndEle && isSelfClosing {
		return false
	}
	...
	// End elements should not have attributes
	if isEndEle && len(attrPart) > 0 {
		return false
	}
...
func isNameString(s string) bool {
	...
	// First character must be in 'first' table
	if !unicode.Is(first, c) {
		return false
	}

	// Remaining characters must be in 'first' or 'second' table
	for n < len(s) {
		s = s[n:]
		c, n = utf8.DecodeRuneInString(s)
